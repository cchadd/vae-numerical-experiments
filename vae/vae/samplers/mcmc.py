...
class HM(BaseMCMCSampler):
    ...
    def sample(self, log_proba, sigma, x0=None, display_progress=True):
        ...
        if x0 is not None:
            ...
            assert x0.size()[-1] == self.dim, f"Starting point lives in dimension {x0.size()[-1]} while dimension {self.dim} is requested"

            
            x[0] = x0

        it = tqdm_notebook(range(1, self.chain_size)) if display_progress else range(1, self.chain_size)
...
class HMC(BaseMCMCSampler):
    ...
    def sample(self, n_lf, log_proba, M=None, x0=None, eps=0.1, acc_temp=1, display_progress=True, skip_acceptance=False, full_trajectory=False):
        """
        ...
        """
        if x0 is not None:
            assert isinstance(x0, torch.Tensor), f"The starting point is not a tensor got {type(x0)} type"
            assert x0.size()[-1] == self.dim, f"Starting point lives in dimension {x0.size()[-1]} while dimension {self.dim} is requested"
        ...
        assert isinstance(M, torch.Tensor), f"Momentum matrix must be a tensor got {type(M)} type"
        assert tuple(M.size()) == (self.dim, self.dim), f"Momentum matrix dimension of [{M.size()[0]}x{M.size()[1]}] while [{self.dim}x{self.dim}] is requested"
        ...
        it = tqdm_notebook(range(0, self.chain_size)) if display_progress else range(0, self.chain_size)

        for i in it:

            x_lf, p_lf = self.__leapfrog(n_lf, log_proba, M, M_inv, x_init)
            if not skip_acceptance:
                H_start = self.__hamiltonian(x_lf[0], p_lf[0], M_inv, M_log_det, log_proba)
                H_end = self.__hamiltonian(x_lf[-1], p_lf[-1], M_inv, M_log_det, log_proba)
    ...
    def __leapfrog(self, n_lf, log_proba, M, M_inv, x0, eps=0.1, display_progress=False, only_final=False):
        """
        ...
        p[0] = torch.distributions.MultivariateNormal(
            ...
            covariance_matrix=M).sample()

        it = tqdm_notebook(range(1, n_lf + 1)) if display_progress else range(1, n_lf + 1)
        for i in it:
            x_ = x[i-1].clone()
    ...
    def __hamiltonian(self, x, p, M_inv, M_log_det, log_proba):
        ...
        return - log_proba(x[None]) + 0.5 * M_log_det + 0.5 * p.T @ M_inv @ p + 0.5 * self.dim * torch.Tensor([2 * np.pi]).log()